...
export async function GET(request: NextRequest) {
  ...
  try {
    // 使用原生 SQL 查詢 MonthlyRepository 表
    let totalQuery = Prisma.sql`SELECT count(*) as total FROM "MonthlyRepository"`;
    let repositoriesQuery = Prisma.sql`SELECT * FROM "MonthlyRepository"`;

    // 添加語言過濾條件
    if (lang) {
      totalQuery = Prisma.sql`SELECT count(*) as total FROM "MonthlyRepository" WHERE language = ${lang}`;
      repositoriesQuery = Prisma.sql`SELECT * FROM "MonthlyRepository" WHERE language = ${lang}`;
    }

    // 添加排序和分頁
    repositoriesQuery = Prisma.sql`
      SELECT * FROM "MonthlyRepository"
    ...
    console.error("獲取每月趨勢資料出錯:", error)
    return NextResponse.json(
      { error: "獲取每月趨勢數據失敗", details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    )
  }
}
