...
export async function GET(request: NextRequest) {
  try {
    ...
    const limit = parseInt(searchParams.get('limit') || '20') // 預設顯示 20 個
    ...
    if (limitedRepositories.length === 0) {
      console.log("沒有找到每日趨勢專案，返回空數組");
    } else {
      console.log(`找到 ${limitedRepositories.length} 個每日趨勢專案`);
    }

    return NextResponse.json(limitedRepositories)
  } catch (error) {
    console.error("API error fetching daily trending repositories:", error)
