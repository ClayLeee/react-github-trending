...
export default function MonthlyPage() {
  ...
  useEffect(() => {
    const fetchRepositories = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/trending/repositories/monthly?${searchParams.toString()}`)
        ...
        console.error("Failed to fetch monthly repositories:", err)
  ...
  const handleLanguageChange = (value: string) => {
    router.push(`/monthly?language=${value}`)
  }
    ...
    return (
      <MainLayout>
        <div className="flex flex-col gap-6">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-gray-800 dark:text-white">月度趨勢</h1>
    ...
    <MainLayout>
      <div className="main-content">
          ...
          <div className="flex flex-col space-y-1">
            <div className="flex items-center gap-2">
              <div className="p-1.5 rounded-lg bg-[#17cfc8]/20 backdrop-blur-sm dark:bg-[#4338ca]/30">
                <Layers className="h-6 w-6 text-[#17cfc8] dark:text-white" />
              </div>
              <h1 className="text-2xl font-bold tracking-tight text-gray-800 dark:text-white">月度趨勢</h1>
            </div>
            <p className="text-muted-foreground dark:text-white/70 font-light tracking-wide">
              探索 GitHub 上最近一個月內新增星星數最多的開源專案
            </p>
          </div>
        ...
        <div className="responsive-grid">
          ...
          <Card className="shadow hover:shadow-lg transition-all hover:scale-[1.02] bg-[#f0fdf4] dark:bg-[#312e81] dark:hover:brightness-110 dark:border-opacity-10 dark:shadow-xl">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-700 dark:text-white">本月新增星數</CardTitle>
              <div className="p-1.5 rounded-full bg-white/50 group-hover:bg-white/70 transition-colors dark:bg-[#4338ca]/30 dark:group-hover:bg-[#4338ca]/50">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
        ...
        <Card className="mt-6 bg-[#f0fdf4] dark:bg-[#312e81] shadow dark:shadow-xl rounded-lg overflow-hidden">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-md transition-colors duration-300 text-gray-800 dark:text-white">月度熱門專案</CardTitle>
              <div className="p-1.5 rounded-full bg-white/50 transition-colors dark:bg-[#4338ca]/30">
                <Layers className="h-4 w-4 text-[#17cfc8] dark:text-white" />
              </div>
            </div>
            <CardDescription className="text-gray-600 dark:text-white/70">
              以本月新增星星數排序，顯示 GitHub 上最近一個月內獲得最多星標的開源專案
            </CardDescription>
          </CardHeader>
          <CardContent>
            <RepositoryTable repositories={repositories} timeRange="monthly" />
          </CardContent>
        </Card>
      </div>
    </MainLayout>
